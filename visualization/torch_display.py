#!/usr/bin/env python3
"""
Throne Room — torch display popup (live dynamics)

  1. CSI subcarriers
  2. Body energy map (rich spatial + labels)
  3. Live time series (mean · energy · spread · pressure · pred)
  4. Aurora / field / host / process status

Usage:
  python -m visualization.torch_display
  python -m observer.startup --torch   # auto multi-process
  python -m observer.startup --full    # torch + Aurora + view + bridge
"""
...
class TorchDisplay:
    def __init__(
        ...
        *,
        ...
        self.action_marks: deque = deque(maxlen=32)
        self.pkt_count = 0
        self._rate = 0.0
        ...
        # 3 · live history — shorter visible window feels more dynamic
        ...
        # filled volumes under energy + pressure for richer dynamics render
    ...
    def _update_map_markers(self, now: float) -> None:
        ...
        for i, ring in enumerate(self._map_rings):
            ...
            a = 0.95 if age < 2 else (0.55 if age < 8 else 0.25)
            ring.center = (cx, cy)
            ring.set_radius(0.025 + 0.07 * min(1.0, energy))
            ring.set_alpha(a)
            ring.set_edgecolor("#ffe066" if age < 2 else ("#4cc9f0" if age < 8 else "#666"))
            short = bid if len(bid) <= 10 else bid[:9] + "…"
            lab.set_position((cx, cy + 0.035 + 0.07 * min(1.0, energy)))
    ...
    def _local_pressure(self) -> float:
        """Instantaneous pressure from live body energies (every UI tick)."""
        if not self.bodies:
            return 0.0
        now = time.time()
        heats = []
    ...
    def tick(self) -> None:
        ...
        # strong local blend so dynamics stay live even when digest is slow
        ...
        # refresh fills for energy + pressure (richer volume render)
        ...
        self.aurora_txt.set_text(
            f"health {health}   pressure {self._last_pressure:.3f}   bodies {field.get('n_bodies', len(self.bodies))}\n"
...
def main() -> None:
    p = argparse.ArgumentParser(description="Throne Room torch display — live dynamics")
