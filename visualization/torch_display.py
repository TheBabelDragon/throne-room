#!/usr/bin/env python3
"""Throne Room closed-loop HUD with always-on multi-head field head.

See repo history / field_head.py for OnlineFieldHead + open_field_head.
"""
...
def _soft_blob(grid, cx, cy, strength, sigma):
    h, w = grid.shape
    xs = (np.arange(w) + 0.5) / w
    ys = (np.arange(h) + 0.5) / h
    X, Y = np.meshgrid(xs, ys)
    grid += strength * np.exp(-((X - cx) ** 2 + (Y - cy) ** 2) / (2 * sigma * sigma))

class TorchDisplay:
    def __init__(self, *, file, udp_port, digest, actions, field_head, head_threshold, size=56, hz=36.0):
        ...
        self.action_marks = deque(maxlen=48)
        self._action_vlines = []
        self._body_flash = {}
        ...
        self.head = None
        try:
            from visualization.field_head import open_field_head
            path = field_head if field_head and Path(field_head).exists() else None
            self.head = open_field_head(path, threshold=head_threshold)
        except Exception as e:
            print(f"[torch-display] field head failed: {e}", flush=True)
            try:
                from visualization.field_head import OnlineFieldHead
                self.head = OnlineFieldHead(threshold=head_threshold)
            except Exception as e2:
                print(f"[torch-display] online head also failed: {e2}", flush=True)
        ...
        self.bars = self.ax_csi.bar(np.arange(32), np.zeros(32), color="#4cc9f0", width=0.85, edgecolor="#0b0d10", linewidth=0.3)
        ...
        self.im_map = self.ax_map.imshow(z, cmap=_THRONE_CMAP, vmin=0, vmax=1, origin="lower", extent=[0, 1, 0, 1], interpolation="bilinear", aspect="equal")
        ...
        for _ in range(12):
            ...
            lab = self.ax_map.text(0.5, 0.5, "", color="#f0f4fa", fontsize=7, ha="center", va="bottom", alpha=0, zorder=7, family="monospace")
            self._map_labels.append(lab)

        self.ax_hist.set_title("3 · live dynamics", color="#e8eef5", fontsize=11, pad=8)
        self.ax_hist.set_xlim(0, 96); self.ax_hist.set_ylim(0, 1.08)
        ...
        self.ax_hist.legend(loc="upper left", fontsize=6, framealpha=0.55, facecolor="#141a24", labelcolor="#ccc", ncol=3)
        ...
        self.aurora_txt = self.ax_aurora.text(0.03, 0.98, "", transform=self.ax_aurora.transAxes, ha="left", va="top", fontsize=8.5, family="monospace", color="#cfe8ff", linespacing=1.45)
        self._chip_patches = []
        self._chip_labels = []
        chip_xy = [(0.05, 0.22), (0.36, 0.22), (0.67, 0.22), (0.05, 0.10), (0.36, 0.10), (0.67, 0.10), (0.36, 0.01)]
        for cx, cy in chip_xy:
            box = FancyBboxPatch((cx, cy), 0.28, 0.09, boxstyle="round,pad=0.02,rounding_size=0.02",
                transform=self.ax_aurora.transAxes, facecolor="#1a2030", edgecolor="#333", lw=1, alpha=0.95)
            self.ax_aurora.add_patch(box)
            self._chip_patches.append(box)
            lab = self.ax_aurora.text(cx + 0.14, cy + 0.045, "", transform=self.ax_aurora.transAxes,
                ha="center", va="center", fontsize=7, family="monospace", color="#8899aa")
            self._chip_labels.append(lab)

        bits = ["live"]
        if self.head:
            bits.append(f"head:{getattr(self.head, 'mode', 'on')}")
        self.fig.suptitle(f"THRONE ROOM  ·  {' + '.join(bits)}  ·  {hz:.0f} Hz HUD", fontsize=13, color="#f0f4fa", fontweight="bold")
    ...
    def _anchor_for(self, body_id):
        if body_id not in self._anchors:
            h = abs(hash(body_id))
            self._anchors[body_id] = (0.11 + (h % 1000) / 1000.0 * 0.78, 0.11 + ((h // 1000) % 1000) / 1000.0 * 0.78)
    ...
    def _ingest(self, pkt):
        ...
        if self.head is not None:
            ...
            feats = [mean, energy, spread, peak, rssi, min(1.0, n_bodies / 6.0), mean_conf, min(1.0, self._rate / 20.0)]
            csi = pkt.get("csi") or self.last_csi
            if csi and len(csi) >= 8:
                try:
                    from observer.multi_head_field import pool_subcarriers
                    pool = pool_subcarriers(csi)
                    feats.extend([pool.fused_mean, pool.fused_energy, pool.fused_spread, pool.entropy,
                                  float(pool.dominant_head) / max(1, len(pool.heads))])
                except Exception:
                    feats.extend([mean, energy, spread, 0.5, 0.0])
            else:
                feats.extend([
                    float(regs.get("head_fused_mean", mean)),
                    float(regs.get("head_fused_energy", energy)),
                    float(regs.get("head_fused_spread", spread)),
                    float(regs.get("head_entropy", 0.5)),
                    float(regs.get("head_dominant", 0.0)),
                ])
    ...
    def _poll_sources(self):
        ...
        for line in alines:
            ...
            self.action_marks.append({"t": tnow, "type": atype, "priority": float(act.get("priority") or 0),
                                       "reason": str(act.get("reason") or "")[:40], "body_id": body_s})
    ...
    def _body_field(self):
        if body_energy_field is not None:
            grid = body_energy_field(self.bodies, self._anchors, size=self.size, prev=self._prev_grid, blend=0.22)
        else:
            ...
            grid = (grid + np.roll(grid, 1, 0) * 0.15 + np.roll(grid, -1, 0) * 0.15 + np.roll(grid, 1, 1) * 0.15 + np.roll(grid, -1, 1) * 0.15) / 1.6
            mx = float(grid.max())
            if mx > 1.0: grid /= mx
        self._prev_grid = grid.copy()
        return grid

    def _update_map_markers(self, now):
        ranked = sorted(self.bodies.items(), key=lambda kv: kv[1]["last"], reverse=True)[:12]
        for i, ring in enumerate(self._map_rings):
            lab = self._map_labels[i]
            if i >= len(ranked):
                ring.set_alpha(0); lab.set_alpha(0); continue
            bid, st = ranked[i]
            cx, cy = self._anchor_for(bid)
            age = now - st["last"]
            ...
            ring.set_alpha(a)
            if flash:
                ring.set_edgecolor("#ff4d6d"); ring.set_linewidth(2.4)
            else:
    ...
    def _set_chips(self, children, host):
        order = [("bridge", "metafield_bridge"), ("view", "throne_view"), ("torch", "torch_display"),
                 ("memory", "metafield_consumer"), ("aurora", "aurora_action"), ("host", None), ("loop", "LOOP")]
        for i, (label, key) in enumerate(order):
            box, lab = self._chip_patches[i], self._chip_labels[i]
            if key is None:
                up = not bool(host.get("stressed")); text = f"host:{host.get('advice') or 'ok'}"
            elif key == "LOOP":
                up = self._loop_closed; text = "loop:CLOSED" if up else "loop:OPEN"
            else:
                st = children.get(key) or {}; up = bool(st.get("alive")); text = f"{label}:{'UP' if up else '—'}"
            box.set_facecolor("#14322a" if up else "#2a1520")
            box.set_edgecolor("#2ecc71" if up else "#555")
            if key == "LOOP" and up:
                box.set_facecolor("#1a3a4a"); box.set_edgecolor("#4cc9f0")
            lab.set_text(text)
            lab.set_color("#a8ffce" if up else "#8899aa")
            if key == "LOOP" and up:
                lab.set_color("#cfe8ff")

    def _local_pressure(self):
        if not self.bodies: return 0.0
        now = time.time()
        heats = []
        for st in self.bodies.values():
            if now - st["last"] > 12: continue
            regs = st["regions"]
            e = float(regs.get("csi_energy", regs.get("csi_mean", 0.0)))
            p = float(regs.get("csi_peak", e))
            heats.append(max(e, p * 0.9))
        if not heats: return 0.0
        total, peak = sum(heats), max(heats)
        active = sum(1 for h in heats if h > 0.12)
        return float(max(0.0, min(1.0, (total / max(1, len(heats))) * 0.45 + peak * 0.40 + min(1.0, active / 4.0) * 0.15)))
    ...
    def tick(self):
        ...
        if len(csi) < 32: csi = list(csi) + [0.0] * (32 - len(csi))
        ...
        self.csi_txt.set_text(f"body {self.last_body}\n{self._rate:.1f} Hz  pkts {self.pkt_count}" + ("  SURPRISE" if surprise else ""))
        ...
        pressure = 0.55 * self._local_pressure() + 0.45 * dig_p
        self._last_pressure = 0.72 * self._last_pressure + 0.28 * pressure
        self.hist_pressure.append(self._last_pressure)

        def _set(line, hist, window=96):
            if not hist: return
            ys = np.asarray(hist, dtype=float); n = min(window, len(ys))
            line.set_data(np.arange(n), ys[-n:])
        for ln, h in ((self.ln_mean, self.hist_mean), (self.ln_energy, self.hist_energy), (self.ln_spread, self.hist_spread),
                      (self.ln_pressure, self.hist_pressure), (self.ln_pred, self.hist_pred), (self.ln_resid, self.hist_resid)):
            _set(ln, h)

        n_show = max(40, min(96, len(self.hist_mean)))
        self.ax_hist.set_xlim(0, n_show)
        try:
            self.fill_energy.remove(); self.fill_pressure.remove()
        except Exception:
            pass
        if self.hist_energy:
            ye = np.asarray(self.hist_energy, dtype=float)[-n_show:]
            self.fill_energy = self.ax_hist.fill_between(np.arange(len(ye)), 0, ye, color="#4cc9f0", alpha=0.18, zorder=1)
        if self.hist_pressure:
            yp = np.asarray(self.hist_pressure, dtype=float)[-n_show:]
            self.fill_pressure = self.ax_hist.fill_between(np.arange(len(yp)), 0, yp, color="#c77dff", alpha=0.22, zorder=1)
        ...
        if self.hist_mean and self.action_marks:
            for act in list(self.action_marks)[-8:]:
                ...
                if age > 25: continue
                x = n_show - age * 4.0
                if x < 0 or x > n_show: continue
                color = {"probe": "#ffe066", "attention": "#4cc9f0", "scale_down": "#f4a261", "hold": "#ff4d6d"}.get(act["type"], "#c77dff")
                self._action_vlines.append(self.ax_hist.axvline(x, color=color, lw=1.4, alpha=0.75, zorder=5))

        resid = float(self.hist_resid[-1]) if self.hist_resid else 0.0
        try:
            if resid > 0.18 or surprise:
        ...
        self._loop_closed = bool(bridge_up and (aurora_up or (now - self._last_action_t) < 30.0) and health in {"ok", "degraded"})
        self._set_chips(children, host)

        head_line = "head: off"
        if self.head:
            mode = getattr(self.head, "mode", "online")
            if self.head.ready:
                head_line = f"head[{mode}] pred={self.head.last_pred:.2f} |r|={self.head.last_abs_residual:.3f}" + (" SURPRISE" if self.head.surprise else "")
            else:
                head_line = f"head[{mode}]: warming"
        ...
        self.status.set_text(
            f"t={now - self._t0:.0f}s  frame={self._frame}  ui≈{1.0/self.tick_dt:.0f}Hz  "
    ...
    def run(self):
        ...
        if self.file: print(f"[torch-display] tail {self.file}", flush=True)
        if self._sock: print(f"[torch-display] UDP :{self.udp_port}", flush=True)
...
def main():
    p = argparse.ArgumentParser(description="Throne Room torch display — head ON multi-head HUD")
    ...
    p.add_argument("--field-head", nargs="?", const=str(DEFAULT_HEAD), default=str(DEFAULT_HEAD),
                   help="Torch ckpt path (optional). Online multi-head head is always on.")
    p.add_argument("--no-head", action="store_true", help="Disable field head")
    p.add_argument("--head-threshold", type=float, default=0.28)
    ...
    fh = None if a.no_head else (Path(a.field_head) if a.field_head else Path(DEFAULT_HEAD))
    TorchDisplay(file=a.file, udp_port=a.udp, digest=a.digest, actions=a.actions,
                 field_head=fh, head_threshold=a.head_threshold, size=a.size, hz=a.hz).run()
