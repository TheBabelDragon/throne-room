"""
Autonomous decision policies over observation digests + recent CSI + head_state.
...
Multi-head aware + head residual coupling closes prediction→action loop.
"""
...
def _read_head_state(path: Path | None = None) -> dict[str, Any]:
    """Couple prediction residual into policy — closes head→action loop."""
    p = path or Path("/tmp/metafield/head_state.json")
...
def decide(
    *,
    ...
    if not bridge.get("alive", True) and csi_lines == 0:
        intents.append(Intent(action="hold", priority=0.9, reason="bridge down and no CSI backlog"))
        return intents

    if health == "degraded":
        intents.append(
            Intent(action="scale_down", priority=0.55, reason="obs path degraded", params={"factor": 0.7})
        )
    ...
    # head residual coupling — prediction AI informs action AI
    head = _read_head_state()
    if head.get("ready") and head.get("surprise"):
        resid = float(head.get("abs_residual") or 0.0)
        intents.append(
            Intent(
                action="probe",
                priority=min(1.0, 0.58 + resid * 0.5),
                reason=f"head surprise |r|={resid:.3f} thr={head.get('threshold')}",
                params={
                    "focus": "head_residual",
                    "abs_residual": resid,
                    "pred": head.get("pred"),
        ...
        resid = float(head.get("abs_residual") or 0.0)
        resid_ema = float(head.get("resid_ema") or 0.0)
        if resid > 0.22 or resid_ema > 0.18:
            intents.append(
                Intent(
                    action="attention",
                    priority=0.42 + min(0.25, resid),
                    reason=f"head residual elevated |r|={resid:.3f}",
                    params={"focus": "head_residual", "abs_residual": resid},
                )
            )
    ...
    for body_id, metrics in series.items():
        ...
        if peak >= 0.75 or (mean >= 0.55 and span >= 0.2):
            ...
            if ent < 0.3 and peak >= 0.6:
                pri = min(1.0, pri + 0.12)
            intents.append(
                Intent(
                    action="probe",
                    priority=pri,
                    reason=f"elevated CSI peak={peak:.2f} mean={mean:.2f} ent={ent:.2f} n={len(recent)}",
                    body_id=body_id,
                    params={
                        "focus": "multi_head" if ents else "csi_energy",
                        "peak": peak, "mean": mean, "entropy": ent, "samples": len(recent),
                    },
                )
            )
